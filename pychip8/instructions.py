...
def store_v(chip8: "Chip8"):
    """ Store registers V0 through Vx in memory starting at location I.

    The interpreter copies the values of registers V0 through Vx into memory, starting at the address in I.
    """

    for i, value in enumerate(chip8.registers.v):
        chip8.memory.ram[chip8.registers.i + i] = value


def ld_v(chip8: "Chip8"):
    ...
    for i in range(0x10):
        chip8.registers.v[i] = chip8.memory.ram[chip8.registers.i + i]
...
def ld_kp(chip8: "Chip8", x: int):
    ...
    if event.type == pygame.KEYDOWN:
        chip8.registers.v[x] = event.key


def skp(chip8: "Chip8", x: int):
    """ Skip next instruction if key with the value of Vx is pressed. """
    keys = pygame.key.get_pressed()
    if keys[chip8.registers.v[x]]:
        chip8.registers.pc += 2
...
def sknp(chip8: "Chip8", x: int):
    ...
    keys = pygame.key.get_pressed()
    if not keys[chip8.registers.v[x]]:
        chip8.registers.pc += 2
...
def draw(chip8: "Chip8", x: int, y: int, n: int):
    ...
    for row in sprite:
        ...
        for col in byte:
            ...
            chip8.registers.v[
                0xF] = old_bit and not chip8.memory.display[index_pos]
